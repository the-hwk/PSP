...
public class NotifyHandler implements Runnable {
    private static final int BUFFER = Integer.parseInt(
            AppConfig.getInstance().getProperty("max_notify_buffer_size")
    );
    ...
    /**
     * Слушает порт для принятия сообщений-уведомлений.
     * Производит обработку взодящих уведомлений
     */
    @Override
    public void run() {
        while (true) {
